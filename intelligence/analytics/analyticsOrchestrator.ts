...
export function runComprehensiveAnalytics(input: AnalyticsInput): ComprehensiveAnalytics {
  ...
  const decisionQualityScore = computeDecisionQualityScore(decisionAnalysis);
  const riskAlerts = detectRiskAlerts(decisionAnalysis, decisions);

  const behavioralAnalysis = analyzeBehavior(events);
  const behavioralInsights = generateBehavioralInsights(behavioralAnalysis);

  // Synthesize recommendations
  const synthesizedRecommendations = synthesizeRecommendations(
    decisionAnalysis,
    riskAlerts,
    behavioralAnalysis,
    behavioralInsights,
...
function synthesizeRecommendations(
  ...
  if (decisionAnalysis.riskTrend > 15) {
    recommendations.push({
      ...
      description:
        'Your decision risk is trending upward. This suggests increasing complexity, pressure, or reduced decision-making quality.',
      actionItems: [
  ...
  if (
    behavioralAnalysis.energyOutcomeCorrelation &&
 ...
 */
export function generateExecutiveSummary(analytics: ComprehensiveAnalytics): string {
  const lines: string[] = [];
  ...
  if (analytics.behavioralAnalysis.bestPerformingTimeBlock) {
    lines.push(
      `✅ Peak Performance: ${analytics.behavioralAnalysis.bestPerformingTimeBlock.label} ` +
