...
exports.generateEnergyInsights = generateEnergyInsights;
function generateEnergyInsights(analysis) {
    const insights = [];
    if (analysis.totalLogs < 5)
        return insights; // require minimum logs
    // 1. Average baseline (LOW priority generally)
    if (analysis.weeklyAverage !== undefined) {
        let interpretation = 'stable';
        if (analysis.weeklyAverage < 4)
            interpretation = 'low';
        else if (analysis.weeklyAverage > 7)
            interpretation = 'high';
